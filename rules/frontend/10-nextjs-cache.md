...
// Fetch đi qua lib/data/analytics.ts — không gọi fetch() trực tiếp (xem rule 11)
...
export async function SummaryCards({ formId }: { formId: string }) {
  const summary: FormSummary = await getAnalyticsSummary(formId)  // ← lib/data, cache: revalidate 300
