...
export function FieldSettings() {
  ...
  // Sync local state when switching to a different field
  useEffect(() => {
    if (field) {
  ...
  const handleLabelBlur = useCallback(() => {
    if (!selectedStepId || !selectedFieldId || !field) return
    ...
    if (trimmed !== field.label) {
      updateField(selectedStepId, selectedFieldId, { label: trimmed })
    }
  }, [selectedStepId, selectedFieldId, field, labelValue, updateField])

  const handleRequiredChange = useCallback(
    (checked: boolean) => {
      if (!selectedStepId || !selectedFieldId) return
      updateField(selectedStepId, selectedFieldId, { required: checked })
    },
    [selectedStepId, selectedFieldId, updateField],
  )

  function commitOptions(options: string[]) {
    if (!selectedStepId || !selectedFieldId) return
    updateField(selectedStepId, selectedFieldId, { options })
  }
  ...
  if (!field || !selectedStepId || !selectedFieldId) {
    return (
